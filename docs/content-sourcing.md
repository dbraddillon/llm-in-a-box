...
`pack.yaml`, written down since `llm-in-a-box` makes it easy to point at arbitrary
URLs.
...
- **Public domain.** Pre-1929 published works, anything explicitly PD-dedicated
  ...
  manuals) — a strong default source for reference/survival packs.
...
- **Wikipedia / Wikibooks / Wikiversity / WikiHow / iFixit** — all CC-BY-SA (WikiHow:
  CC-BY-NC-SA). A Kiwix ZIM of one of these is a common offline-content format;
  extracting text from one for a pack carries the same license as browsing it live.
...
3. If it's `type: http`, does the site's ToS say anything about bulk/automated
   download?
4. If it's `type: local` pointing at a folder someone else already assembled, don't
   assume the license call was already made for you — re-check it per item.
